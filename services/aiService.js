    ...
    hasName && hasPhone
      ...
      : hasPhone
        ...
        : hasName
          ...
          : "Имени и телефона нет. Коротко ответь по сути запроса без цены и сразу попроси имя + номер WhatsApp.",
    "Запрещено называть цену, стоимость или «от X ₸», пока клиент явно не спросил (сколько стоит / цена / стоимость).",
    "Правильный пример: «Поможем с настройкой рекламы в Google и TikTok. Напишите, пожалуйста, как к вам обращаться и номер WhatsApp.»",
    "Неправильный пример: «Реклама от 100 000 ₸. Как вас зовут?»",
...
function buildWebsiteSystemInstructions(lead = {}) {
  return [
    ...
    "",
    "Правило цен (жёстко):",
    "- Не называй цену, стоимость и «от X ₸» в первом ответе и пока клиент явно не спросил.",
    "- Если клиент не спрашивал цену — отвечай по сути услуги и веди к имени + WhatsApp.",
    "- Цену называй только когда клиент прямо спрашивает: «сколько стоит», «цена», «стоимость», «от скольки».",
    "",
    "Цены (только если клиент спросил; другие не выдумывай):",
    "Сайт под ключ — 50 000 ₸",
    "Сайт + реклама — 150 000 ₸",
    "Ведение рекламы — от 100 000 ₸/мес (НЕ 70 000 — эта цена устарела)",
    ...
    "Ответь по сути запроса и веди к следующему шагу (имя + WhatsApp, если их ещё нет).",
...
export function buildDynamicLeadBlock(lead = {}, extras = {}) {
  ...
  return [
    ...
    extras.channel === "web"
      ? "1. На сайте правила WEBSITE CHAT перекрывают мини-бриф системного промпта. Не называй цену, пока клиент явно не спросил. Остальные запреты CREOLAB (выдумки, «передам менеджеру») действуют."
...
export function buildAiInput({ knowledgeBase, history, message, lead, extraInstruction, channel }) {
  ...
  return [
    ...
    "",
    channel === "web"
      ? "=== АКТУАЛЬНЫЕ ЦЕНЫ ДЛЯ САЙТА (если клиент спросил цену) ===\nСайт — 50 000 ₸\nСайт + реклама — 150 000 ₸\nВедение рекламы — от 100 000 ₸/мес\nНикогда не называй 70 000 ₸ за рекламу."
      : "",
    "",
